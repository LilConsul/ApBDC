...
#define LIS3MDL_REG_CTRL2        0x21   // Control register 2 (Full-scale, reboot, soft reset)
...
static LIS3MDL_StatusTypeDef LIS3MDL_ReadMultipleRegisters(uint8_t reg, uint8_t *buffer, uint16_t length);
...
LIS3MDL_StatusTypeDef LIS3MDL_Init(void) {
    LIS3MDL_ConfigTypeDef default_config = {
        .full_scale = LIS3MDL_FS_4_GAUSS,
...
LIS3MDL_StatusTypeDef LIS3MDL_InitWithConfig(const LIS3MDL_ConfigTypeDef *config) {
    ...
    if (who_am_i != LIS3MDL_WHO_AM_I_VALUE) {
        trace_printf("LIS3MDL: Device not found! WHO_AM_I=0x%02X (expected 0x3D)\n", who_am_i);
    ...
    if (status != LIS3MDL_OK) return status;
    ...
    uint8_t ctrl3_value = (config->mode == LIS3MDL_MODE_CONTINUOUS) ? 0x00 : 
                          (config->mode == LIS3MDL_MODE_SINGLE) ? 0x01 : 0x03;
...
LIS3MDL_StatusTypeDef LIS3MDL_ReadRawData(LIS3MDL_RawDataTypeDef *data) {
    ...
    status = LIS3MDL_ReadMultipleRegisters(LIS3MDL_REG_OUT_X_L, raw_buffer, LIS3MDL_XYZ_REG_SIZE);
    if (status != LIS3MDL_OK) {
        return status;
    }
...
LIS3MDL_StatusTypeDef LIS3MDL_SetMode(LIS3MDL_ModeTypeDef mode) {
    uint8_t ctrl3_value = (mode == LIS3MDL_MODE_CONTINUOUS) ? 0x00 : 
                          (mode == LIS3MDL_MODE_SINGLE) ? 0x01 : 0x03;
...
static LIS3MDL_StatusTypeDef LIS3MDL_ReadMultipleRegisters(uint8_t reg, uint8_t *buffer, uint16_t length) {
    ...
    i2c_status = I2C_DMA_ReadBuffer(lis3mdl_device_address, reg, buffer, length);
