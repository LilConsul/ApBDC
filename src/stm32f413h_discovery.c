...
/** @defgroup STM32F413H_DISCOVERY_LOW_LEVEL_Private_TypesDefinitions STM32F413H
 * Discovery Low Level Private Typedef
 * @{
 */
typedef struct {
    __IO uint16_t REG;
...
/** @defgroup STM32F413H_DISCOVERY_LOW_LEVEL_Private_Defines STM32F413H
 * Discovery Low Level Private Def
 * @{
 */
/**
...
#define __STM32F413H_DISCOVERY_BSP_VERSION                                     \
    ((__STM32F413H_DISCOVERY_BSP_VERSION_MAIN << 24) |                         \
...
/** @defgroup STM32F413H_DISCOVERY_LOW_LEVEL_Private_Variables STM32F413H
 * Discovery Low Level Variables
 * @{
 */

const uint32_t GPIO_PIN[LEDn] = {LED3_PIN, LED4_PIN};
...
/** @defgroup STM32F413H_DISCOVERY_LOW_LEVEL_Private_FunctionPrototypes
...
FMPI2Cx_ReadMultiple(FMPI2C_HandleTypeDef *fmpi2c_handler, uint8_t Addr,
                     uint16_t Reg, uint16_t MemAddSize, uint8_t *Buffer,
                     uint16_t Length);
static HAL_StatusTypeDef
FMPI2Cx_WriteMultiple(FMPI2C_HandleTypeDef *fmpi2c_handler, uint8_t Addr,
                      uint16_t Reg, uint16_t MemAddSize, uint8_t *Buffer,
                      uint16_t Length);
...
/** @defgroup STM32F413H_DISCOVERY_LOW_LEVEL_Private_Functions STM32F413H
 * Discovery Low Level Private Functions
 * @{
 */
...
FMPI2Cx_ReadMultiple(FMPI2C_HandleTypeDef *fmpi2c_handler, uint8_t Addr,
    ...
    status = HAL_FMPI2C_Mem_Read(fmpi2c_handler, Addr, (uint16_t)Reg,
                                 MemAddress, Buffer, Length, 1000);
...
FMPI2Cx_WriteMultiple(FMPI2C_HandleTypeDef *fmpi2c_handler, uint8_t Addr,
    ...
    status = HAL_FMPI2C_Mem_Write(fmpi2c_handler, Addr, (uint16_t)Reg,
                                  MemAddress, Buffer, Length, 1000);
...
static void FMC_BANK3_MspInit(void) {
    ...
    /* GPIOD configuration: GPIO_PIN_7 is  FMC_NE1 , GPIO_PIN_11 ans GPIO_PIN_12
     * are PSRAM_A16 and PSRAM_A17 */
...
void AUDIO_IO_Write(uint8_t Addr, uint16_t Reg, uint16_t Value) {
    ...
    FMPI2Cx_WriteMultiple(&hI2cAudioHandler, Addr, Reg,
                          FMPI2C_MEMADD_SIZE_16BIT, (uint8_t *)&Value, 2);
}
...
uint16_t AUDIO_IO_Read(uint8_t Addr, uint16_t Reg) {
    ...
    FMPI2Cx_ReadMultiple(&hI2cAudioHandler, Addr, Reg, FMPI2C_MEMADD_SIZE_16BIT,
...
uint16_t TS_IO_ReadMultiple(uint8_t Addr, uint8_t Reg, uint8_t *Buffer,
                            uint16_t Length) {
    return FMPI2Cx_ReadMultiple(&hI2cAudioHandler, Addr, (uint16_t)Reg,
...
void TS_IO_WriteMultiple(uint8_t Addr, uint8_t Reg, uint8_t *Buffer,
                         uint16_t Length) {
    FMPI2Cx_WriteMultiple(&hI2cAudioHandler, Addr, (uint16_t)Reg,
                          I2C_MEMADD_SIZE_8BIT, Buffer, Length);
}
...
uint8_t TS_IO_Read(uint8_t Addr, uint8_t Reg) {
    ...
    FMPI2Cx_ReadMultiple(&hI2cAudioHandler, Addr, Reg, I2C_MEMADD_SIZE_8BIT,
