...
WIFI_Status_t WIFI_OpenClientConnection(uint32_t socket, WIFI_Protocol_t type,
    ...
    conn.Type = (type == WIFI_TCP_PROTOCOL) ? ES_WIFI_TCP_CONNECTION
                                            : ES_WIFI_UDP_CONNECTION;
...
WIFI_Status_t WIFI_StartServer(uint32_t socket, WIFI_Protocol_t protocol,
    ...
    conn.Number = socket;
    conn.LocalPort = port;
    conn.Type = (protocol == WIFI_TCP_PROTOCOL) ? ES_WIFI_TCP_CONNECTION
                                                : ES_WIFI_UDP_CONNECTION;
